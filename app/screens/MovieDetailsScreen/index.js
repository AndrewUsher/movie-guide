...
import { Alert, Share, ScrollView, View, Text, Image, TouchableOpacity } from 'react-native'
...
export default class MovieDetailsScreen extends Component {
  ...
  convertRatingToStars(vote_average) {
    ...
    return vote_average <= 5
      ...
      : [...Array(length)].map((e, i) => (
          <FontAwesome key={i} name="star" size={width * 0.06} color="#fff" style={{ marginRight: 5 }} />
        ))
  }
  ...
  renderErrorMessage = () => <Error icon="alert-octagon" action={this.requestMoviesInfo} />
        ...
        {isLoading ? (
          ...
          <ScrollView>
            <View style={styles.containerMainPhoto}>
              <Image source={this.getImageApi(backdrop_path)} style={styles.mainPhoto} resizeMode="cover" />
              {video && video.site === 'YouTube' && (
                <TouchableOpacity activeOpacity={0.5} style={styles.play} onPress={this.actionPlayVideo}>
                  <FontAwesome name="play" size={width * 0.07} color="#fff" style={{ marginLeft: 5 }} />
                </TouchableOpacity>
              )}
              <TouchableOpacity
              ...
              >
                <View style={styles.containerBackgroundPhotoInfo}>
                  ...
                  <View style={styles.photoStar}>{this.convertRatingToStars(vote_average)}</View>
                </View>
              </TouchableOpacity>
            </View>
            <View style={styles.containerMovieInfo}>
              <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={styles.movieFirstInfo}>
                <View style={styles.movieInfo}>
                  ...
                  <Text style={styles.subTitleInfo}>{this.convertMinsToHrsMins(runtime)}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Genre</Text>
                  <Text style={styles.subTitleInfo}>{this.convertToGenre()}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Language</Text>
                  <Text style={styles.subTitleInfo}>{this.convertToUpperCaseFirstLetter(original_language)}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Release</Text>
                  <Text style={styles.subTitleInfo}>{this.convertToDate(release_date)}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Budget</Text>
                  <Text style={styles.subTitleInfo}>{this.convertToDolar(budget)}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Revenue</Text>
                  <Text style={styles.subTitleInfo}>{this.convertToDolar(revenue)}</Text>
                </View>
                <View style={styles.movieInfo}>
                  <Text style={styles.titleInfo}>Adult</Text>
                  <Text style={styles.subTitleInfo}>{this.convertAdult(adult)}</Text>
                </View>
              </ScrollView>
              <View style={styles.movieSecondInfo}>
                ...
                >
                  <Text style={styles.subTitleInfo}>{overview || 'Uninformed'}</Text>
                </ReadMore>
              </View>
              <View style={styles.movieSecondInfo}>
            ...
            {images.length ? (
              <SlideImages showImage={showImage} images={images} actionClose={this.actionImage} />
            ) : null}
          </ScrollView>
        )}
        <TeamDetail
